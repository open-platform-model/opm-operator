...
- **Known deviation — local dev/e2e fixtures:** `task dev:test:local`, the e2e-local flow, `make run` / `make publish-test-module`, and `.tasks/module.yaml` / `.tasks/release.yaml` publish and resolve `opmodel.dev/modules/test/*` fixtures (e.g. `hello`) via the local registry. The fixtures are not local-only: the release and e2e workflows publish them to GHCR (pre-release tags), so GHCR carries them too — the local flows just publish their own copies instead of depending on that. This stays localhost-mapped until the fixtures move under `testing.opmodel.dev/*`. Local-registry-gated: start `opm-registry` only when running these flows. Plain `task dev:test` and CI use GHCR.
