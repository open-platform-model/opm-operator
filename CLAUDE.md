...
- **Renders share nothing; the kernel gate is narrow.** The process has one library Kernel. Its context-owning calls (module acquisition, instance synthesis, on-disk acquisition, the platform build) are serialised behind `Store.AcquireKernel`; `Kernel.Render` builds in its own context and runs outside the gate, bounded only by `--max-concurrent-renders` (default 1, sized by memory: `docs/RENDERING.md`). A render leases the generated platform record so the Platform reconciler's prune skips the directory it reads. Never hold the gate across a render.
