...
var _ = Describe("ModuleInstance platform-gated rendering", func() {
	const namespace = "default"

	newModuleInstance := func(ctx context.Context, name string) types.NamespacedName {
	...
	Context("when no platform is materialized (5.2)", func() {
		It("blocks with PlatformNotReady, applying and pruning nothing", func() {
			...
			nn := newModuleInstance(ctx, "gate-block-mr")
			...
			// PlatformNotReady is a transient blocked-on-dependency state, so it
			// requeues on the fast exponential backoff (BackoffBaseDelay for the
