...
// stubFetcher creates a placeholder instance.cue at dir/<path>/instance.cue so
...
func (s *stubFetcher) Fetch(_ context.Context, _ string, _ string, dir string, _ opmsource.FetchOptions) error {
	...
	return os.WriteFile(filepath.Join(target, "instance.cue"), []byte("package instance\n"), 0o644)
}
...
var _ = Describe("ModulePackage Controller", func() {
	...
	createModulePackage := func(ctx context.Context, name, path string, suspend bool, dependsOn []fluxmeta.NamespacedObjectReference) *releasesv1alpha1.ModulePackage {
	...
	buildReconciler := func(fetcher opmsource.Fetcher, renderer render.PackageRenderer) *ModulePackageReconciler {
	...
	reconcileTwice := func(ctx context.Context, r *ModulePackageReconciler, nn types.NamespacedName) reconcile.Result {
	...
	Context("Source not ready", func() {
		It("sets Ready=False with SourceNotReady and requeues", func() {
			...
			r := buildReconciler(&stubFetcher{}, &stubPackageRenderer{})
			nn := types.NamespacedName{Name: name, Namespace: namespace}
			result := reconcileTwice(ctx, r, nn)
			Expect(result.RequeueAfter).To(BeNumerically(">", time.Duration(0)))
	...
	Context("Path not found", func() {
		It("sets Stalled=True with PathNotFound when instance.cue missing", func() {
			ctx := context.Background()
			name := "path-missing"
			...
			createModulePackage(ctx, name, "does/not/exist", false, nil)

			fetcher := &stubFetcher{skipWriteFile: true}
			r := buildReconciler(fetcher, &stubPackageRenderer{})
			nn := types.NamespacedName{Name: name, Namespace: namespace}
	...
	Context("Suspend", func() {
		It("sets Suspended reason without reconciling", func() {
			...
			createModulePackage(ctx, name, "releases/app", true, nil)

			r := buildReconciler(&stubFetcher{}, &stubPackageRenderer{err: fmt.Errorf("should not render")})
			nn := types.NamespacedName{Name: name, Namespace: namespace}
			_ = reconcileTwice(ctx, r, nn)
		...
		It("resumes reconciliation when suspend flips to false", func() {
			...
			rel := createModulePackage(ctx, name, "releases/app", true, nil)
	...
	Context("Unsupported kind", func() {
		It("sets Stalled=True with UnsupportedKind", func() {
			...
			fetcher := &stubFetcher{pathInArtifact: "releases/app"}
			renderer := &stubPackageRenderer{err: fmt.Errorf("%w: kind \"SomeOtherKind\"", render.ErrUnsupportedKind)}
			r := buildReconciler(fetcher, renderer)
			nn := types.NamespacedName{Name: name, Namespace: namespace}
			_ = reconcileTwice(ctx, r, nn)
