...
var _ = Describe("Release platform-gated rendering", func() {
	...
	newRelease := func(ctx context.Context, name string) types.NamespacedName {
	...
	Context("when a platform is materialized (5.1)", func() {
		It("renders the release and applies the resulting resources", func() {
			...
			// The render+apply success path runs through the ReleaseRenderer seam
			// the cut-over swaps to KernelReleaseRenderer. Exercising the kernel's
			...
			// succeeds (the platform-present case), mirroring the ModuleRelease gate
			// test.
			src := readySource(ctx, "gate-apply-rel-src", "main@sha256:a1", "sha256:a1")
			nn := newRelease(ctx, "gate-apply-rel")
	...
	Context("when no platform is materialized (5.2)", func() {
		It("blocks with PlatformNotReady, applying and pruning nothing", func() {
			...
			nn := newRelease(ctx, "gate-block-rel")
			...
			reconciler := &ReleaseReconciler{
				...
				Fetcher:         &stubFetcher{pathInArtifact: "releases/app"},
				Renderer:        &stubReleaseRenderer{err: render.ErrPlatformNotReady},
			}
	...
	Context("when the platform changes (5.3)", func() {
		It("re-enqueues every Release in the cluster", func() {
			ctx := context.Background()

			nn1 := newRelease(ctx, "gate-enqueue-rel-1")
			nn2 := newRelease(ctx, "gate-enqueue-rel-2")
			...
			reqs := reconciler.mapPlatformToReleases(ctx, platform)
			...
			Expect(k8sClient.Delete(ctx, &releasesv1alpha1.Release{
