...
func reportRenderDiagnostics(ctx context.Context, tracker *WarningTracker, recorder events.EventRecorder, obj client.Object, result *render.RenderResult) {
	...
	if result.PlatformIdentity != "" {
		key := keyOf(obj)
		log.Info("Rendered against platform package",
			"name", key.Name, "namespace", key.Namespace, "platformIdentity", result.PlatformIdentity)
	}
	if len(result.ResolvedVersions) > 0 {
		log.V(1).Info("Resolved module versions", "resolvedVersions", result.ResolvedVersions)
	}
}
