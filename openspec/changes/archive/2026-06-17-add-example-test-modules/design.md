...
### D7: redis headless Service required a catalog extension (catalog@v0.6.0)
The Context above assumed the pinned catalog could already model everything these examples need. Implementation disproved that for **one** case: the catalog's `#ExposeSchema` only allowed `type: "ClusterIP" | "NodePort" | "LoadBalancer"` with an allocated cluster IP — there was **no way to emit `clusterIP: "None"`**, so a true headless governing Service (D5, idiomatic for a StatefulSet) was not expressible in any published catalog version (verified against `v0.5.1`, `v0.5.2`, `v0.5.7`, `v0.6.0-dev`). Everything else redis needs (StatefulSet, standalone PVC, exec probe) was already modellable.

Resolution (operator's choice when the gap surfaced): extend the catalog additively rather than relax the spec. `catalog_opm` gained an optional `expose.clusterIP: "None"` field on `#ExposeSchema`, wired through the service-transformer; omitting it preserves the prior virtual-IP behavior, so the change is backward-compatible for existing consumers. This ships as catalog **`opmodel.dev/catalogs/opm@v0.6.0`** (additive `feat:` → minor bump). *Alternatives considered:* (a) author redis with a plain ClusterIP Service and weaken the spec — rejected, headless is the correct StatefulSet pattern and a single field closes the gap; (b) drop redis from this change — rejected, the catalog fix is small.

**Cross-repo dependency this introduces:** `podinfo` and `redis` pin `opmodel.dev/catalogs/opm@v0.6.0`. `podinfo` does not *use* the new field but is re-pinned to `v0.6.0` so it materializes against the same catalog version the e2e `Platform` resolves (newest-in-range), avoiding the catalog-version skew the matcher is sensitive to. **Release ordering:** catalog `v0.6.0` must be published to GHCR (from `catalog_opm`) before this change's CI module-publish can resolve these modules. The PVC also renders as a *standalone* `PersistentVolumeClaim` (the catalog's pvc-transformer), not a StatefulSet `volumeClaimTemplate`; the spec admits either.
