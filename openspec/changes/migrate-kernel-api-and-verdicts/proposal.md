...
The operator pins `library v1.0.0-alpha.26` and cannot build against `library` main. Two breaking library changes have landed there since:
...
Both land in the same library alpha, and the operator cannot compile between them: `internal/render/kernel_module_renderer.go` is edited by both. That is why this is one change rather than two.
...
**Affected files (3 import a deleted helper package, 4 more read a moved or reshaped type):**
...
**Tests:** `internal/reconcile/resolution_test.go` constructs the over-subscription cause. Any test asserting the kernel's exact warning wording moves to the operator's wording. The e2e and Flux fixture suites exercise render output, which this change does not alter.
