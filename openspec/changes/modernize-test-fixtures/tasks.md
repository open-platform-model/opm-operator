...
- [x] 1.3 Regenerate `test/fixtures/modules/hello/cue.mod/module.cue` deps via `cue mod tidy`: pins `opmodel.dev/core@v0 v0.4.0` + `opmodel.dev/catalogs/opm@v0 v0.5.0` (catalog published locally first — see 2.1). Required a `cue` CLI at v0.17.x; the system v0.16.1 cannot read the v0.17.0 catalog.
...
- [~] 5.2 `task dev:test:local` — **blocked by a pre-existing production bug, not by the fixture.** The module fixture is correct (concrete-evaluates standalone) and the `KernelModuleRenderer` happy-path now *runs* (no longer skips). But `acquire` and the renderer happy-path fail because `internal/moduleacquire/shim.go`'s `import mod; mod` embedding cannot load a core@v0 `#Module` (self-referential `metadata.modulePath`/`version` → `field not allowed`). Tracked in the `fix-moduleacquire-core-v0` follow-up. `dev:test:local` goes green once that lands.
