# operator-version-identity (delta)

The operator's build-time version identity: a source-burned constant maintained by release automation, exposed at startup and via `Platform.status.operatorVersion`. Slice A6 of enhancement 0006 (D24, operator half).
