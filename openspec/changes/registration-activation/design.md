...
- [A field index on `spec.providerRef` is new machinery in this controller] -> optional; the requirement is only that an unrelated instance does not enqueue every claim. A filtered list is acceptable at the expected scale and can be indexed later.
