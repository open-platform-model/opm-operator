...
<!-- Renamed from `release-artifact-loading` (0002 D2/D10); the spec dir is `git mv`'d at archive. Authored file convention `release.cue` → `instance.cue` (D9). -->

### Requirement: Artifact unpacking and path navigation
The ModulePackage reconciler MUST fetch the Flux source artifact, unpack it to a temporary directory, and navigate to `spec.path` within the extracted tree to locate the instance CUE package.

#### Scenario: Valid path with instance.cue
- **WHEN** the artifact is unpacked and `spec.path` resolves to a directory containing `instance.cue`
...
#### Scenario: Path exists but no instance.cue
- **WHEN** `spec.path` resolves to a directory that does not contain `instance.cue`
- **THEN** the reconciler sets `Ready=False` with reason `InstanceFileNotFound` and `Stalled=True`

### Requirement: CUE evaluation with registry resolution
The ModulePackage reconciler MUST evaluate the CUE package at `spec.path` using `CUE_REGISTRY` set from the controller's `--registry` flag or `OPM_REGISTRY` environment variable.
...
- **THEN** the reconciler receives a concrete CUE value representing the instance
...
The ModulePackage reconciler MUST clean up the temporary directory used for artifact extraction after CUE evaluation completes, regardless of success or failure.
...
The ModulePackage reconciler MUST NOT require `cue.mod/module.cue` at the artifact root. The CUE module structure is expected at `spec.path`, not at the root of the Flux artifact.
