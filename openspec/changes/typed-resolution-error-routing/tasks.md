...
- [ ] 1.1 Add the shared typed helper in `internal/reconcile` (design D1):
...
- [ ] 1.2 `classifyRenderError` (`internal/reconcile/moduleinstance.go`): call the helper
...
- [ ] 1.3 `renderErrorReason` (`internal/reconcile/modulepackage.go`): call the same helper
      after the `ErrUnsupportedKind` check and ahead of the `isResolutionErrorMsg` string
      fallback.
- [ ] 1.4 Verify joined-error behavior: `*oerrors.UnresolvedDemandsError` joined with
...
- [ ] 2.1 Unit tests (new — no classifier tests exist today): bare and wrapped
...
- [ ] 2.2 Check `internal/controller/moduleinstance_platform_gate_test.go:152`'s
...
- [ ] 3.1 `task dev:fmt dev:vet dev:lint dev:test`.
- [ ] 3.2 Scan commit messages / PR body for bare `@` tokens; plain co-author trailer only.
