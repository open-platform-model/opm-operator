...
registry as a library `*module.Module`. Acquisition wraps the shared
`*kernel.Kernel`: it generates a minimal shim package that depends only on the
target module, loads it through the kernel with a per-call registry override, and
decodes it into a library module — without pinning a catalog version or mutating
