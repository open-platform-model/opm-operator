...
Define how the operator treats a `ModuleInstance` it does not own. The `spec.owner` marker lets the OPM CLI deploy a `ModuleInstance` (recording its own inventory in `status`) without the operator fighting it for the same resources: when an instance is CLI-owned the operator stays hands-off (no render, apply, prune, deletion cleanup, or finalizer) and records a single `ManagedExternally` acknowledgement. Flipping the marker to `operator` cleanly hands the instance over to a normal reconcile.
