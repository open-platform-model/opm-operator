...
// These tests exercise KernelReleaseRenderer — the Release CR render path —
// against the hello release fixture (test/fixtures/releases/hello). That fixture
// is an author-written #ModuleRelease that imports the published hello module and
// embeds it via #module. It is the registry-backed sibling of the module-renderer
// integration test; both render the same ConfigMap, one via Go synthesis
// (ModuleRelease) and one via an authored release.cue (Release).
//
// Embedding a published #Module under #ModuleRelease.#module re-unifies the
// closed module against #Module. This only loads on a core@v0 whose #Module
...
var _ = Describe("KernelReleaseRenderer Integration", func() {
	Context("when the platform store is empty", func() {
		It("returns ErrPlatformNotReady or ErrUnsupportedKind before compiling", func() {
			...
			fixtureDir, err := filepath.Abs(filepath.Join("..", "..", "fixtures", "releases", "hello"))
	...
	Context("when the store holds a materialized platform", func() {
		...
		BeforeEach(func() {
			...
			// Pin the catalog subscription to the version the hello fixture module
			// targets (test/fixtures/modules/hello pins catalogs/opm@v0.5.1).
			// Resource/transformer FQNs are version-qualified, so a floating
			// subscription that resolves a different catalog version would leave
			// the component unmatched — independent of the render-path fix.
			catalogRange := os.Getenv("OPM_TEST_CATALOG_RANGE")
			if catalogRange == "" {
				catalogRange = ">=0.5.1 <0.5.2"
			}
		...
		It("loads the authored release fixture (imported #module) and renders its resources", func() {
			fixtureDir, err := filepath.Abs(filepath.Join("..", "..", "fixtures", "releases", "hello"))
			Expect(err).NotTo(HaveOccurred())
			...
			Expect(kind).To(Equal(render.KindModuleRelease))
			Expect(res).NotTo(BeNil())
			Expect(res.Resources).NotTo(BeEmpty(),
				"the hello fixture must compile to at least one resource (a ConfigMap)")
